...
def do_main():
    ...
    clan_id = (sys.argv[1])
    terminal_ts = _journal_monitor.timestamp_n_days_ago(2)
    ...
    with open(csv_file_summary, "w", encoding='utf-8') as f:
        for name in attack_metadata.keys():
            line = (
                ...
                f"{attack_metadata[name]['units'][2]},{compute_ratfuck_quotient(attack_metadata[name]['points'])}\n")
            f.write(line)
    ...
    try:
        ...
        while True:
            ...
            if not j_result and try_count < 3:
                print(f"no jresult.  try count {try_count}")
                try_count += 1
                time.sleep(5)
                continue
